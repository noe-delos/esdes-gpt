...
const SuggestionButton: React.FC<SuggestionButtonProps> = ({
  ...
  <motion.button
    ...
    whileTap={{ scale: 0.97 }}
    className="px-4 py-2 rounded-full bg-[#00788A]/20 text-[#00788A] hover:bg-[#00788A]/10 text-sm flex items-center gap-2"
    onClick={onClick}
  >
...
const SendButton: React.FC<SendButtonProps> = ({
  ...
  <motion.button
    ...
    type={type}
    disabled={disabled}
    className="rounded-full bg-[#00788A] hover:bg-[#e95136] h-10 w-10 flex items-center justify-center text-white disabled:cursor-not-allowed"
  >
...
const MarkdownContent = memo(({ content }: MarkdownContentProps) => (
  <ReactMarkdown
    ...
    components={{
      ...
      th: ({ children }) => (
        <th className="px-4 py-2 bg-zinc-50 text-zinc-800">{children}</th>
      ),
      td: ({ children }) => (
        <td className="px-4 py-2 text-zinc-700">{children}</td>
      ),
      tr: ({ children }) => <tr>{children}</tr>,
      pre: ({ children }) => (
        <pre className="bg-zinc-50 rounded-lg p-4 overflow-x-auto">
          {children}
        </pre>
      ),
...
export default function Page() {
  ...
  const customSubmit = (e: FormEvent) => {
    ...
    // Get current message count from localStorage - using userMessageCounthec as the key
    const currentCount = parseInt(
      localStorage.getItem("userMessageCounthec") || "0"
    );
    ...
    localStorage.setItem("userMessageCounthec", newCount.toString());
  ...
  const suggestionData: SuggestionItem[] = [
    {
      ...
      query:
        "Trouve des alumni qui travaillent chez Boston Consulting Group (BCG)",
      color: "#0b48c2",
    },
    {
      text: "Promotion 2022",
  ...
  const renderToolResult = (
    ...
    if (
      (toolName === "searchAlumni" || toolName === "getRandomAlumni") &&
      state === "result" &&
      result
    ) {
      let title =
        toolName === "getRandomAlumni"
          ? "Alumnis aléatoires"
      ...
      return (
        <AlumniTable
          key={`${toolCallId}-${idx}`}
          alumni={result as Partial<Alumni>[]}
          title={title}
        />
      );
    }
    ...
    <div className="flex flex-col h-full w-full" ref={rootElementRef}>
      ...
      <div className="w-full relative">
        <div className="max-w-5xl mx-auto px-4 py-3">
          {/* Logo and title */}
          <div
            className="flex items-center space-x-3 fixed top-3 left-4 z-10 cursor-pointer"
            onClick={() => router.push("/")}
          >
            <Image
              src="/logo.png"
              ...
              <div className="py-20 text-center max-w-4xl mx-auto">
                ...
                >
                  ...
                  >
                    ...
                    <div className="absolute left-3 bottom-4 bg-[#00788A]/20 text-[#00788A] rounded-full px-3 py-1 flex items-center gap-2 text-sm pointer-events-none">
                      <Image
                        src="/logo-small.png"
                        alt="logo"
                        width={18}
                  ...
                  {messages.map((message: any, index: number) => {
                    ...
                    return (
                      <motion.div
                        ...
                        onMouseEnter={() =>
                          !isUser && setHoveringMessage(message.id)
                        }
                        ...
                        >
                          {isUser ? (
                            ...
                            <div className="flex items-start gap-3">
                              <ShimmeringTitle className="text-black font-thin">
                                Chargement
                              </ShimmeringTitle>
                            </div>
                          ) : (
      ...
      <p className="text-zinc-400 font-light fixed bottom-0 mb-4 left-[45%] text-xs">
        contact:{" "}
        <a href="mailto:paul@ks-entreprise.com" className="">
          paul@ks-entreprise.com
        </a>
      </p>
    </div>
  );
